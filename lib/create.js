...
module.exports = function(config, allDone) {
  ...
  const runContainer = (host, cConfig, done) => {
    ...
    cmd(host, `(docker-srm ${cConfig.args.name} || true) && docker run ${args} ${cConfig.image} ${cConfig.command || ''}`, done);
  };
  ...
  async.auto({
    ...
    dns: ['ec2', (results, done) => {
      route53.run(results.ec2.host, 'create', done);
    }],
